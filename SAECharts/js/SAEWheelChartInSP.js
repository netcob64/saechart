...
function SAESPWheelChart(p) {
    ...
    this.load = function(loadHandler, loadErrorHandler, siteUrl, spListName, spListItemTitle) {
        ...
        SAESPinitSPLibAndLoadScript(function() {
            ...
            try {
                ...
                clientContext.load(me.collListItem, 'Include(Title, Id, data, Options, Nom, Ann_x00e9_es,Mois, Nombre_x0020_Dataset,Debloquer)');
        ...
        try {
            ...
            while (listItemEnumerator.moveNext()) {
                ...
                if (title == me.spListItemTitle) {
                    chartData = oListItem.get_item('data');
                    options = oListItem.get_item('Options');
                    try {
                    ...
                    me.name = oListItem.get_item('Nom');
                    year = oListItem.get_item('Ann_x00e9_es');
                    month = oListItem.get_item('Mois');
                    nbDataset = oListItem.get_item('Nombre_x0020_Dataset');
                    unlock = oListItem.get_item('Debloquer');
                    me.dataId = id;
            ...
            alert('Erreur: Proleme avec la liste sharepoint ' + me.siteUrl + '/[Listes] ' + me.spListName + ' ' + me.spListItemTitle);
        }
    ...
    this.save = function() {
        ...
        oListItem.set_item('data', me.chart.getData());
        var optionToSave = me.options;
        delete optionToSave.date;
        delete optionToSave.nbDataSet;
        oListItem.set_item('Options', JSON.stringify(optionToSave));
        oListItem.update();
    ...
    this.createAndSave = function() {
        ...
        me.oListItem.set_item('Title', this.year + '-' + this.month + this.titlePostFix);
        me.oListItem.set_item('data', me.chart.getData());
        me.oListItem.set_item('Mois', month);
        me.oListItem.set_item('Nom', me.name);
        me.oListItem.set_item('Nombre_x0020_Dataset', me.defaultNbDataSet);
        me.oListItem.set_item('Ann_x00e9_es', parseInt(me.year));
        me.oListItem.set_item('Debloquer', false);
        if (me.options != -1) {
            var optionToSave = me.options;
            delete optionToSave.date;
            delete optionToSave.nbDataSet;
            me.oListItem.set_item('Options', JSON.stringify(optionToSave));
        }
    ...
    this.loadTrend = function(loadTrendHandler, siteUrl, spListName, spListItemTitle) {
        ...
        SAESPinitSPLibAndLoadScript(function() {
            ...
            try {
                ...
                camlQuery.Query = '<OrderBy><FieldRef Name="Title"  Ascending="TRUE" /></OrderBy>';
                ...
                alert("Erreur: Probleme au chargement des donnes de la liste sharepoint " + me.siteUrl + '/[Listes] ' + me.spListName + ' ' + me.spListItemTitle);
            }
        });
    }
    this.onLoadTrendQuerySucces = function(sender, args) {
        ...
        try {
            ...
            alert('Erreur au chargement des donnes du graph : "' + me.spListItemTitle + '" from ' + me.siteUrl + '/' + me.spListName);
